...
const ActionTable = ({unique_key, setAction, buttonLabel, cell_value,
  ...
  action_query, showActionList,setVisibilityActions,handleSetAction,
  default_actions, actions,indexTextsearch}) => {
    console.log("actionResults",actionResults);
    console.log("actionQueryResults",actionQueryResults);
  ...
  // detach table
  const [tablecontent, setEmpty] = React.useState(actionQueryResults);
  ...
  if (datastory_data.dynamic_elements && datastory_data.dynamic_elements.length) {
    datastory_data.dynamic_elements.forEach(element => {
      default_actions = [];
      if (element.type == 'textsearch' && element.position == indexTextsearch) {
  ...
  const [updatedActions,setUpdatedAction] = React.useState(saved_actions);
      ...
      datastory_data.dynamic_elements.forEach(element => {
        if (element.type == 'textsearch' && element.position == indexTextsearch) {
          if (element.textsearch) {
            ...
            setUpdatedAction(saved_actions);
           }
        }
      })
    };
    console.log("actions refreshed",saved_actions);
  }
                  ...
                  {!finalpreview && default_actions.length > 0 && (
                    ...
                    {!finalpreview && showActionListTable ?
                      <><ColumnListActions
                          ...
                          column_name={heading}
                          setAction={setAction}/>
                        <span className="refresh" onClick={refreshActions}> Restart the search to see the changes</span>
                      </>
                      : <></>}
                  </th>)
                )}</tr>
...
const ColumnListActions = ({index, default_actions,
                            handleSetAction, actions, column_name,setAction}) => {

  // retrieve all actions that could be applied to the column header
  // save them in an array of arrays: default_actions
  ...
  // look in the saved actions for each column,
  ...
  // isActive is an array of action names selected for the column at hand
  ...
  // when de/selected by the user in column headers: setActive
  // and update the list of saved actions for the column at hand: actions
...
const TextSearchResults = ({ index , queryResults , queryString , setResults,
  ...
  actionResults,setActionResult}) => {
  console.log("actions in first table",actions);
...
const TextSearch = ({ unique_key, index ,
    ...
    React.useEffect(() => {
      try {
        ...
        console.log("actions in text search",actions);
